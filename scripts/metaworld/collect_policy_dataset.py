...
def collect_dataset(benchmark, names, path, train=True, tasks_per_env=10, expert_ep=5, within_env_ep=5, cross_env_ep=10, random_ep=2, epsilon=0.1, noise_type='gaussian'):
    ...
    for name in names:
        ...
        for i, task in enumerate(tasks):
            ...
            dataset = ReplayBuffer(dest_env.observation_space, dest_env.action_space, capacity=total_ep_per_env*502, cleanup=False)
...
if __name__ == "__main__":
    ...
    if args.rank == 0:
        ...
        seed(args.rank)
        collect_dataset(benchmark, names, args.path, train=not args.valid, tasks_per_env=args.tasks_per_env, 
                        expert_ep=args.expert_ep, within_env_ep=args.within_env_ep, cross_env_ep=args.cross_env_ep, 
                        random_ep=args.random_ep, epsilon=args.epsilon, noise_type=args.noise_type)
        # Done.
