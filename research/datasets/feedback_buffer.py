...
import math
import io
from .replay_buffer import save_episode, load_episode, construct_buffer_helper
from research.utils.utils import get_from_batch

class FeedbackLabelDataset(torch.utils.data.IterableDataset):

    def __init__(self, observation_space, action_space, path, discount=0.99, segment_size=20, 
                        nstep=1, batch_size=64, use_next_obs=False,
                        replay_capacity=100000, segment_capacity=100000, cleanup=False,
                        preload_path=None):
        super().__init__()
        ...
        self.obs_1_buffer = construct_buffer_helper(observation_space, self.segment_capacity, begin_pad=(self.segment_size,))
        self.obs_2_buffer = construct_buffer_helper(observation_space, self.segment_capacity, begin_pad=(self.segment_size,))
        self.action_1_buffer = construct_buffer_helper(action_space, self.segment_capacity, begin_pad=(self.segment_size,))
        self.action_2_buffer = construct_buffer_helper(action_space, self.segment_capacity, begin_pad=(self.segment_size,))
    ...
    def _get_segment(self):
        episode = None
        ep_len = 0
        while episode is None or ep_len - self.segment_size*self.nstep < 1: # Ensure that the episode is also long enough!
        ...
        action_segment = {k:v[idx:idx+self.segment_size*self.nstep] for k, v in episode["action"].items()} \
                         if isinstance(episode["action"], dict) else episode["action"][idx:idx+self.segment_size*self.nstep]
        if self.use_next_obs:
            idx += 1
        obs_segment = {k:v[idx:idx+self.segment_size*self.nstep] for k, v in episode["obs"].items()} \
                        if isinstance(episode["obs"], dict) else episode["obs"][idx:idx+self.segment_size*self.nstep]
        if self.nstep > 1:
            action_segment = {k:v[::self.nstep] for k, v in action_segment.items()} if isinstance(action_segment, dict) else action_segment[::self.nstep]
            obs_segment = {k:v[::self.nstep] for k, v in obs_segment.items()} if isinstance(obs_segment, dict) else obs_segment[::self.nstep]
        # Run np sum prod
    ...
    def get_segments(self, batch_size=64):
        ...
        return dict(obs_1=obs[:batch_size], action_1=action[:batch_size], reward_1=reward[:batch_size], state_1=state[:batch_size],
                    obs_2=obs[batch_size:], action_2=action[batch_size:], reward_2=reward[batch_size:], state_2=state[batch_size:])
...
class CrossPolicyOracleFeedbackDataset(torch.utils.data.IterableDataset):
    ...
    def __init__(self, observation_space, action_space, paths, segment_capacity, **kwargs):
        ...
        self._task_to_id = {task: index for index, task in enumerate(task_names)}
        self._id_to_task = {v:k for k, v in self._task_to_id.items()}
        self._datasets = {task: FeedbackLabelDataset(observation_space, action_space, p, 
                                                    segment_capacity=segment_capacity, **kwargs) for task, p in zip(task_names, paths)}
        ...
        for task, dataset in self._datasets.items():
            ...
            for _ in range(segment_capacity):
                ...
                if random.random() > 0.5:
                    label = 1
                    batch = dict(obs_1=negative_segment['obs_1'], action_1=negative_segment['action_1'],
                                 obs_2=positive_segment['obs_2'], action_2=positive_segment['action_2'])
                else:
                    label = 0
                    batch = dict(obs_1=positive_segment['obs_1'], action_1=positive_segment['action_1'],
                                 obs_2=negative_segment['obs_2'], action_2=negative_segment['action_2'])
                dataset.label_segments(batch, label)
            # Now remove all the loaded episodes to save memory
            dataset._episodes = {}
            print("Finished", task)
