...
class ActorCriticRewardPolicy(ActorCriticPolicy):
    ...
    def __init__(self, observation_space, action_space, 
                       actor_class, critic_class, reward_class, encoder_class=None, 
                       actor_kwargs={}, critic_kwargs={}, encoder_kwargs={}, reward_kwargs={}, **kwargs):
        super().__init__(observation_space, action_space, actor_class, critic_class, encoder_class=encoder_class,
                         actor_kwargs=actor_kwargs, critic_kwargs=critic_kwargs, encoder_kwargs=encoder_kwargs, **kwargs)
    ...
    def reset_reward(self, device=None):
        observation_space = self.encoder.output_space if hasattr(self.encoder, "output_space") else self.observation_space
        reward_class = vars(research.networks)[self.reward_class] if isinstance(self.reward_class, str) else self.reward_class
